...
| `secrets-scan.yml` (Secret Scanning & Security) | PR (TruffleHog diff-only), push to `main` (full history), **weekly cron Mon 02:00 UTC**, dispatch | Six jobs: TruffleHog OSS (`--only-verified`), gitleaks **CLI** (pinned release, checksum-verified, full history, `.gitleaks.toml` allowlists the documented test vectors), checkov over the template corpus (deep scan is V08 on rendered output), committed-state/plan check, dependency review (PR-only), summary | All open-source scanners in normal jobs — no paid product. **Never use `gitleaks/gitleaks-action`**: since v3 the action demands a paid license key for organization repos; the CLI is MIT and free (first-run failure, 2026-08-18). The cron fires on its own once the workflow is enabled |
