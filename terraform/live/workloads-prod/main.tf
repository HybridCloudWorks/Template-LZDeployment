...
locals {
  ...
  # remote-state read. try() rather than a direct attribute read: a
  # connectivity state applied before that output existed does not carry it,
  # and this is a lookup against a value already in state — not a provider
  # read, which try() could not rescue.
...
#
# ONE INSTANCE PER REGION, never per spoke. Both calls take the module's
# composed name, stflowlogs${region_code}${environment}, and storage account
# names are globally unique — so any other instance in the same
# (region, environment) must supply storage_account_name explicitly or it
# collides estate-wide, not just within this stack (contract 8a). The
# connectivity layer's hub instances do exactly that; do not add an override
# here, which would rename storage accounts an applied estate already holds.
#
