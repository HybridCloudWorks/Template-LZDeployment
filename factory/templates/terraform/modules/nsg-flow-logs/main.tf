...
resource "azurerm_storage_account" "flow_logs" {
  ...
  #checkov:skip=CKV2_AZURE_40:Shared-key authorization is NOT disabled here, unlike the state account. Whether Network Watcher can write flow logs to a shared-key-disabled account is unverified, and flipping it on an unverified basis risks silently breaking flow-log delivery. Tracked as TODO item 2.17, which names what would settle it.
